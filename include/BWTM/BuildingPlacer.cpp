  ...
  if (!type.isRefinery())
  {
    for(int x = startx; x < endx; x++)
	{
		...
		{     
			...
			BWAPI::TilePosition b = BWTA::getNearestBaseLocation(x,y)->getTilePosition();
			// Make sure we leave room for a possible expansion
			if (sqrt( (double) ( (x-b.x())*(x-b.x()) + (y-b.y())*(y-b.y()) ) ) < buildDist + width + BWAPI::UnitTypes::Terran_Command_Center.tileWidth())
			{
				return false;
			}
		}
